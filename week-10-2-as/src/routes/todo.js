...
var authMiddleware = function (req, res, next) {
    ...
    router.post("/signup", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
        var result, user, _a, response, token, error_1;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    result = verifySignup.safeParse(req.body);
                ...
                case 3:
                    _a = result.data;
                    return [4 /*yield*/, bcrypt_1.default.hash(result.data.password, 10)];
                case 4:
                    _a.password = _b.sent();
                    response = prisma.user.create({
                        data: __assign({}, result.data),
                        select: {
                            id: true,
                            email: true,
                        },
                    });
                    token = jsonwebtoken_1.default.sign({ email: result.data.email }, "secret");
                    res.json({
